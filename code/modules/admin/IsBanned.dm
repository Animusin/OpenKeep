	...
	if(!real_bans_only && !C && CONFIG_GET(flag/usewhitelist))
		if(!check_whitelist(ckey))
			if (admin)
				...
				return list("reason"="whitelist", "desc" = "\nBecome whitelisted! https://discord.gg/qZw267fFsX")
/*
#ifdef MATURESERVER
	if(!check_whitelist(ckey))
		...
		if(num >= 10)
			if(check_patreon_lvl(ckey) < 2)
				...
				return list("reason"="trialexpire", "desc" = "\nBecome whitelisted to continue playing here! https://discord.gg/qZw267fFsX")
#endif
*/
/*	//Blacklist
...
*/
	...
	if(!real_bans_only && !C)
		if (!admin)
			if(get_playerquality(ckey) <= -100)
				log_access("Failed Login: [ckey] - account_age")
				return list("reason"="pqlow", "desc"="\nYou have completed the game!")
